...
var TaskWhere = struct {
	...
	TaskID:      whereHelperstring{field: "\"task\".\"task_id\""},
	DisplayName: whereHelperstring{field: "\"task\".\"display_name\""},
...
var (
	taskAllColumns            = []string{"task_id", "display_name", "created_at", "updated_at"}
	taskColumnsWithoutDefault = []string{"display_name"}
	taskColumnsWithDefault    = []string{"task_id", "created_at", "updated_at"}
...
func FindTask(ctx context.Context, exec boil.ContextExecutor, taskID string, selectCols ...string) (*Task, error) {
	taskObj := &Task{}
	...
	query := fmt.Sprintf(
		"select %s from \"task\" where \"task_id\"=$1", sel,
	)
...
func TaskExists(ctx context.Context, exec boil.ContextExecutor, taskID string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"task\" where \"task_id\"=$1 limit 1)"
